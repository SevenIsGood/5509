...
//////////////////////////
	#define _CHIP_AGPIOEN_ADDR       (0x4400u)
	#define _CHIP_AGPIOEN_SET(Val)\
	     _PREG_SET(_CHIP_AGPIOEN_ADDR, Val)

	#define _CHIP_AGPIODIR_ADDR       (0x4401u)
	#define _CHIP_AGPIODIR_SET(Val)\
	     _PREG_SET(_CHIP_AGPIODIR_ADDR, Val)

	#define _CHIP_AGPIODATA_ADDR       (0x4402u)
	#define _CHIP_AGPIODATA_SET(Val)\
	     _PREG_SET(_CHIP_AGPIODATA_ADDR, Val)
	#define _CHIP_AGPIODATA_GET\
     _PREG_GET(_CHIP_AGPIODATA_ADDR)
	/////////////////

#define AGPIOEN1  *(int *)0x4400u
#define AGPIODIR1 *(int *)0x4401u
#define AGPIODATA1 (*(volatile ioport Uint16*)(0x4402u))
...
void main(void)
{
	...
	/*EMIF为DATA EMIF*/
    CHIP_RSET(XBSR,0x0a00);
	CHIP_RSET(AGPIOEN,0x02);
	CHIP_RSET(AGPIODIR,0x02);
	CHIP_RSET(AGPIODATA,0x02);
