...
void main(void)
{
	 ...
	 /*CLS¿âµÄ³õÊ¼»¯£¬ÕâÊÇ±ØÐèµÄ*/
	 CSL_init();
	
	 /* Set IVPH/IVPD to start of interrupt vector table */
	 /*ÐÞ¸Ä¼Ä´æÆ÷IVPH£¬IVPD£¬ÖØÐÂ¶¨ÒåÖÐ¶ÏÏòÁ¿±í*/
	 IRQ_setVecs((Uint32)(&VECSTART));
	 
	 /* Temporarily disable all maskable interrupts */
	/*½ûÖ¹ËùÓÐ¿ÉÆÁ±ÎµÄÖÐ¶ÏÔ´*/
	old_intm = IRQ_globalDisable();   
	
	/* Open Timer 0, set registers to power on defaults */
	/*´ò¿ª¶¨Ê±Æ÷0£¬ÉèÖÃÆäÎªÉÏµçµÄµÄÄ¬ÈÏÖµ£¬²¢·µ»ØÆä¾ä±ú*/
	...
	/*ÉèÖÃ¼Ä´æÆ÷ST1µÄINTMÎ»£¬Ê¹ÄÜËùÓÐµÄÖÐ¶Ï*/
	IRQ_globalEnable();      
