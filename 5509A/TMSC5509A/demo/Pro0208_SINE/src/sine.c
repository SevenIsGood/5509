...
/*																		    */
/****************************************************************************/
static void GenSineData(Uint32 sampleRate, Uint32 freqTONE, Uint32 amplify, Uint32 *Data_out, Uint32 NumOfUnit)
{
	int i;
	float pi = 3.1415926;
	float w = 0.0;
	Uint32 *pDataOut = 0;
	amplify = (amplify) >> 1 ;
	pDataOut = (Uint32 *)Data_out;
	w =2 * freqTONE * pi / sampleRate;
	for(i = 0; i < NumOfUnit; i++)
	{
		pDataOut[i] = ((Uint32)(amplify * sin(w*i)) + amplify);
	}
...
main()
{
    ...
    GenSineData(10000, 1000, 1000, nSine, SINE_LEN);
	while ( 1 )
	{
