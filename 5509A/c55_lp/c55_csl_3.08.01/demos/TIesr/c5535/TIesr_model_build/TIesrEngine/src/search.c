 ...
 *
 *  Modifications to operate on 16-Bit Plattforms
...
static void coloring_beam(unsigned short cur_time_index, gmhmm_type *gv)
{
  ...
  if (cur_time_index == UNDEFINEDTIME) return ; /* no beam cell has this time (no sym was collected at this time) */
...
{
  short i, t;
  unsigned short beam_idx, s_next, count=0, active_count = 0;
  ...
  /* mark all active beam cells in the backtrace of the current beam cell
   linked list pointed to by the current time cell. */
  ...
  /* set available(un-marked) to 0x7fff, and un-mark the occupied (marked) */
   ...
   {
      if( p->idx_flag & 0x8000 )
      {
         /* This beam cell is marked as currently active */
         p->idx_flag &= 0x7fff; /* marked -- unmark it*/
         active_count++;
      }
      else
      {
   ...
   /* Compressed number of beam cells now active */
   gv->active_beam_cells = active_count;
#ifdef DEBUGP
  ...
  */
  active_count = 0;
   ...
   //pos means the next available, so pos-1 is the current
   /* Check all possibly active time cells */
   for( t = gv->evalstat.time.high_pos - 1; t >= 0; t-- )
   {

      beam_idx = time_base[t].beam_idx;

      /* If time cell t is currently set as used, compress the associated beam 
       cell linked list to just the currently active beam cells */
      if( beam_idx != USHRT_MAX )
      {
         /* Follow the beam cell linked list until the first non empty cell
         is located. It will become new head of the time cell entry. */
         for(; NOT_END_IX_CELL( bm[beam_idx].next, gv->max_beam_cell ); beam_idx = bm[beam_idx].next )
         {
            if( !CELLEMPTY( bm[beam_idx].idx_flag ) ) break;
         }

         if( CELLEMPTY( bm[beam_idx].idx_flag ) )
         {
            /* the list contains only empty beam cells; reuse the time cell */
            time_base[t].beam_idx = USHRT_MAX;
            /* Set pointer to lowest unused time cell */
            gv->evalstat.time.cur_pos = t;
         }
         else
         {
            /* Active head beam cell found. The list contains at least one cell.
             Update beam linked list to include only active cells */
            for( head = bm + beam_idx; NOT_END_IX_CELL( head->next, gv->max_beam_cell ); head = bm + head->next )
            {
               // find the next active beam cell in the list
               for( s_next = head->next, p = bm + s_next; CELLEMPTY( p->idx_flag ) && NOT_END_IX_CELL( p->next, gv->max_beam_cell ); s_next = p->next, p = bm + s_next );

               /* If end of active beam cells found, terminate the linked list*/
               if( CELLEMPTY( p->idx_flag ) )
               {
                  // stopped because of empty cell, that the end of the list
                  head->next = gv->max_beam_cell;
                  break;
               }
                  /* Extend the linked list to include the next active beam cell */
               else head->next = s_next;
            }

            /* Update the time cell with the head of the beam cell linked list */
      ...
      {
         /* The  time cell does not contain an active beam cell list index, so the
       time cell can be reused */
         gv->evalstat.time.cur_pos = t;
      }
   }

  /* Compressed number of time cells now active */
  gv->active_time_cells = active_count;
}
...
//if (gv->frm_cnt>0 )printf("===================now,word %d \n", sym);
  ...
  gv->beam_base[idx].idx_flag = (p_cell->sym == USHRT_MAX)?  gv->max_beam_cell  //zlook
          : find_beam_index(p_cell->sym, STATE_FRM(p_cell->frm), gv, last_frm);  //previous stats' symble and starting frame.
...
static void
within_model_path( unsigned short hmmcode, HmmType *hmm, StateCell *p_prev, StateCell *p_cur, gmhmm_type *gv, short prune )
{
   long cur_prob;
   short *trnprob;
   short i, j;
   ...
   n1 = n_state - 1;
   pprev = p_prev;
   for ( i = 0; i < n1; i++, trnprob += n_state, pprev++ )
   { /*for all states in the HMM*/

      if ( pprev->scr > prune )
      { /*if a possible start -- within model prune*/

         /*for all target states within model*/
         for ( j = 0; j < n1; j++ )
         {

            /* Add duration penalty to transition probability 
               if not silence HMM and is a loop transition */
            if ( hmmcode != 0 && i == j )
            {
               dur = STATE_DUR( pprev->frm );
               tprob = LONG2SHORT( (long) trnprob[j] + (long) dur * TRAN_DUR_BIAS );
            }
            else
            {
               tprob = trnprob[j];
            ...
            /* there exist i to j jump. note transprob is updated above with i */
            if ( tprob > BAD_SCR )
            {

               state_cell = p_cur + j;
               cur_prob = (long) pprev->scr + (long) tprob;

               /* Viterbi decoding done here on transition probability */
               if ( cur_prob > (long) state_cell->scr )
               {
                  state_cell->scr = (short) cur_prob;

                  /* propogate previous symbol */
                  state_cell->sym = pprev->sym;

                  /* Propagate previous symbol frame, and state duration */
                  if ( hmmcode != 0 && i == j )
                  {
                     /* loop transition in non-silence HMM */
                     dur++;
                     dur = MIN( dur, 15 );
                     state_cell->frm = STATE_CAT( dur, pprev->frm );
                  }
                  else
                  {
                     /* Set last frame count only, which clears duration count */
                     state_cell->frm = STATE_FRM( pprev->frm );
                  }
               }
  ...
  for (i = 0; i < nbr_next; i++) {  /*for all posible next symbols*/
	...
	if (bIsWord && gv->bUseNBest ){
	  ...
	  nb_update_nbest(gv->ptr_nbest_tmp->pNBestCells, uBeamIdx, start_cell, v_sym, cSym,  
        ...
        /* Viterbi decoding is done here in transitioning from the exit state
         of the prior symbol's HMM into a state of a next grammar symbol HMM.
         Store the frame index and the symbol that started evaluation of this
         next HMM. Depending on the type of search (WORD or HMM) the symbol
         that started evaluation may be the prior symbol or prior
         word-ending symbol. */
	if ( start_scr >= (long) start_cell->scr ) {

	  start_cell->scr = (short) start_scr;
	  start_cell->frm = STATE_FRM(starttime); 
	  start_cell->sym = v_sym;  
	}
#ifdef USE_NBEST
...
//p_cur=NULL;
	 ...
	 return eTIesrEngineEvalCellOverflow;
       ...
       within_model_path(hmmcode, hmm, p_prev, p_cur, gv, prune); /*hmm, last state cell, new allocated state cell, ...*/
...
// old:	  status = cross_model_path(exit_prob + gv->tranwgt, v_sym, t_sym, hmm_dlt, sym_dlt, cur_idx, frm_cnt,gv, sym_prev->depth);
	...
	if (gv->word_backtrace==WORDBT){ 
	 ...
	 { starttime = STATE_FRM(p_prev[ n_state - 1 ].frm);
  ...
  update_obs_prob(short mfcc_feature[], short cur_idx, unsigned short frm_cnt, gmhmm_type *gv, unsigned short last_frm)
{
  ...
  unsigned short last_SYM = USHRT_MAX, last_FRM = UNDEFINEDTIME;
...
//printf("=== gs %d, code: %d, pdf %d feat: %d %d %d\n", gs, v_symcode, pdf_idx, sObs[0], sObs[1], sObs[18]);	   
	...
	//if ( sym_cell->depth > (gv->bestdepth +9) || sym_cell->depth < (gv->bestdepth -10)  ) scr-=500;
	  ...
	  if ( scr < BAD_SCR ) state_cell->scr = BAD_SCR;
	     ...
	     */
            frm = STATE_FRM( state_cell->frm );
	    if ( frm != UNDEFINEDTIME ) 
	      if ( (frm != last_FRM) || (state_cell->sym != last_SYM)) {  //if it is from a new search node (frame/sym pair)
		...
		if (gv->need_compact==TRUE ) //|| gv->word_backtrace==FALSE) //if (gv->evalstat.beam.cur_pos > 1000)   //if (gv->frm_cnt==14)
		  {   
		  	mark_cells_state( frm, state_cell->sym, gv, last_frm);   
		  }   
		last_FRM = frm; last_SYM = state_cell->sym; 
	      }
	  }
...
//if (frm_cnt<9) printf ("=========curbest %d\n",gv->best_cur_scr);
...
#endif
          /* Check if there is a transition to the exit state. */
	  if ( *trnprob > BAD_SCR ) {  

	    scr = (long) state_cell->scr  +  (long) *trnprob;

            /* Viterbi decode here for the highest likelihood state transition
             to the exit state.*/
	...
	sumdepth+=sym_cell->depth;  n_actsym++;      
        ...
        /* If a valid transition to the exit state exists, update the exit
         state cell with the information corresponding to the highest
         likelihood exit transition. */
	if ( max_exit_scr > BAD_SCR ) {
	  state_cell->scr = max_exit_scr;
	  state_cell->sym = max_exit_state->sym;
	  state_cell->frm = STATE_FRM(max_exit_state->frm);
	
	  /* if end of sym, put it in the beam cell list if it is word ending
      or if HMM backtrace information needed */
...
init_search(short mfcc_feature[], short cur_idx, gmhmm_type *gv, unsigned short last_frm)
{
  ...
  /* Initialize active beam and time cell counters */
  gv->active_beam_cells = 0;
  gv->active_time_cells = 0;
  ...
  //printf("init===gender %d... nhmm %d   hmset %d \n", hmm_dlt, gv->trans->n_hmm, gv->trans->n_hmm_set );
	...
	return eTIesrEngineEvalCellOverflow;
      ...
      for (j = 0; j < n_state - 1; j++) {          
        ...
        /* Check if state has initial entry probability */
	if ( startprob[ j ] > BAD_SCR ) {          

          /* Determine initial state likelihood score */
	  pdf_idx = hmm->pdf_idx[ j ];
	...
	*/
	  ...
	  mlp_update(gv, hmmcode , scr);
#endif
          /* If a transition from state to exit state exists, determine its
           likelihood.*/
	  if ( trnprob[ j * n_state + exit_state ] > BAD_SCR ) {
        ...
        /* Set prior symbol and frame for the state to invalid values indicating
         beginning of search. */
	state_cell[ j ].frm = UNDEFINEDTIME;     
	state_cell[ j ].sym = USHRT_MAX;
      } /* end of all emiting states */

      /* Set prior symbol and frame of exit state to invalid values */
      state_cell[ exit_state ].frm = UNDEFINEDTIME; 
      state_cell[ exit_state ].sym = USHRT_MAX;

       /* If valid likelihood of exit, update exit state likelihood score. */
      if ( max_exit_scr > BAD_SCR ) {   
	...
	gv->ending_sym_count ++;
        ...
        /* Add the symbol/prior info of the exit state to the beam cell linked
         list for this frame if HMM is word ending or if HMM backtrace requested */
   if (psym->n_next & 0x8000 || gv->word_backtrace!=WORDBT)
   {
     status = grow_beam_cell(VTL_SYM(t_symcode, sym_dlt), state_cell + exit_state, first_time_call, 0,
                             gv, last_frm);
...
{ 
  ...
  /*track the last beam cell index allocated this frame. Used to update the
    next pointer within the beam cells during allocation and at end of frame. */
  ...
  /* --------------------------------------------------
    ...
    tmp = (long)gv->best_prev_scr + (long)gv->prune * gv->nbr_dim;
#ifdef MODEL_LEVEL_PRUNE
    ...
    if (frm_cnt<PRUNE_FRAME_TH  && frm_cnt>1 )  //note frm_cnt start from 1 in this part. 20  30 40
    {
     // thresholdd 6000 is the pruning at 20 frame. Note that 1280 is about equivalant to PRUNE=1 before (with 10 mfcc), so 6000 is about PRUNE=5
      	tmp+= (long)(PRUNE_FRAME_TH -frm_cnt)* PRUNE_TIME_FACTOR; // 300; // *6000/20.0;  //6000 tuned with 3000 4000 ....  110000, 10000 seems best for dim10  9000 for 9dim.  1000 is for clean
    }
    else  // The program set prune, which is PRUNE=7, is applied here. 
     tmp = (long)gv->best_prev_scr + (long)gv->prune * gv->nbr_dim; //no need. Set above already.
#endif    	
	...
	long tmp_l =((long)tmp_s * powt[frm_cnt])>>14;  //14 is from powt, which is 0.9^n * 2^14
	tmp_s= (short) tmp_l;
	...
	if ( gv->beam_count > tmp_s )  
      ...
      tmp=tmp > (long)gv->best_prev_scr -PRUNE_MINIMUM ? (long)gv->best_prev_scr -PRUNE_MINIMUM: tmp; //PRUNE_MINIMUM  2560
   ...
   /*for FA*/     
    if (gv->word_backtrace == HMMFA) tmp = gv->best_prev_scr - (long)150 * gv->nbr_dim;
...
//printf("prune is %d\n", prune);    
    ...
    /* Compact beam/time cells if too many used */
      if( gv->need_compact == TRUE ) // || gv->word_backtrace==FALSE )
      {
         //printf( "\ncompact start: frame: %d beam count: %d  time count: %d\n",
         //   gv->frm_cnt, gv->active_beam_cells, gv->active_time_cells );

         compact_beam_cells( gv->beam_base, gv->time_base, gv );
         gv->need_compact = FALSE;

         //printf( "compact end: frame: %d beam count: %d  time count: %d\n\n",
         //   gv->frm_cnt, gv->active_beam_cells, gv->active_time_cells );
      }
      else
      {

#ifdef USE_BEAM_FRACTION
         /* Check if used beam cell buffer is above a defined threshold */
         th_n =
            (short)( ( (long) gv->max_beam_cell * BEAM_COMPACT_FRACTION ) >> 15 );
         if( gv->active_beam_cells >= th_n )
         {
            gv->need_compact = TRUE;
         }
#else
         /* Check and set to compact if beam cell overflow is predicted to occur
          in less than BEAM_COMPACT_FRAMES from current frame */
         if( gv->max_beam_cell - gv->active_beam_cells - (int)gv->beam_count*BEAM_COMPACT_FRAMES <= 0 )
         {
            gv->need_compact = TRUE;
         }
#endif

         /* Since at most one time cell is consumed per frame, compact if
          the next frame uses last available time cell. */
         if( gv->active_time_cells >= gv->max_time_cell - 1  )
         {
            gv->need_compact = TRUE;
         }
      }
...
/*-----------------------------------------------------------------------------
 ...
 Report the HMM state log likelihood corresponding to a particular grammar 
 symbol, or all grammar symbols if symbol_code < 0.  This function can be the
 basis of analyzing the per-frame scores of active HMMs. The function should
 be called during recognition after each frame has been processed by a call to
 search_a_frame.
 ----------------------------------------------------------------------------*/
TIESRENGINECOREAPI_API TIesrEngineStatusType
symbol_status( gmhmm_type *gv, short symbol_code )
{
#ifdef DEBUG_STATES
   unsigned short v_sym, t_sym, hmmcode, sym_dlt, hmm_dlt;
   SymCell *sym_cell, *ps0 = (SymCell *)NULL, *ps1 = (SymCell *)NULL;
   HmmType *hmm;
   short n_state;
   StateCell *p_state;
   SymType *psym;
   unsigned short frame_type;
   unsigned short nbr_sym[NSYM];
   short direction, cls;
   EvalIdx *eval_idx;
   short state;
   short scr;

   /* The current odd/even frame type, 0=even 1=odd */
   frame_type = gv->frm_cnt & 0x1;

   /* Pointer to current EvalIdx structure which provides the number of 
    currently active cells in the symbol and state cell arrays */
   eval_idx = gv->eval + frame_type;

   /* Array of number of active symbols for each class. Currently 
    there are two classes, typically male and female. */
   for( cls = 0; cls < NSYM; cls++ )
      nbr_sym[cls] = eval_idx->cur_sym[cls];

   /* Direction to traverse the symbol cell sub-arrays for each class */
   direction = frame_type ? -1 : 1;

   /* Find the last entered symbol cells in each symbol cell sub-array, if 
    any were allocated. */
   if( nbr_sym[0] ) ps0 = SYM_CELL( gv->symcell_base, nbr_sym[0] - 1, frame_type, gv->max_sym_cell2 );
   if( nbr_sym[1] ) ps1 = SYM_CELL( gv->symcell_base + gv->max_sym_cell2, nbr_sym[1] - 1, frame_type, gv->max_sym_cell2 );


   /* No symbol cells active. Should not happen during recognition. */
   if( ( !ps0 ) && ( !ps1 ) )
   {
      return eTIesrEngineAlignmentFail;
   }

   /* Iterate through all active grammar symbol cells for the current frame */
   while( ps0 || ps1 )
   {

      /* Get the next active symbol cell based on the state cell index to process
         through the state cell array. */
      SET_SYM_CELL( ps0, ps1, sym_cell, nbr_sym[0], nbr_sym[1], direction, ps0->start_cell > ps1->start_cell );


      /* If this symbol is active then proceed to process it */
      if( !( sym_cell->sym_code & 0x8000 ) )
      {
         /* The class-dependent virtual symbol for this symbol cell */
         v_sym = sym_cell->sym_code;

         /* loop over all possible classes to find the true grammar symbol index
          and the HMM class delta for the corresponding HMM. This translates 
          from a gender dependent symbol code to gender independent  sym type
          code in the grammar network. */
         for( hmm_dlt = 0, sym_dlt = 0, t_sym = v_sym; t_sym >= gv->trans->n_sym;
            t_sym -= gv->trans->n_sym, hmm_dlt += gv->trans->n_hmm_set, sym_dlt += gv->trans->n_sym );

         /* If this symbol is the requested symbol to process, or if the user
             wishes to process all symbols, then output HMM state data for
             the symbol */
         if( symbol_code < 0 || symbol_code == t_sym )

         /* From the true grammar symbol, determine the corresponding HMM,
             adjusted for class (gender) */
         psym = (SymType *)( gv->base_net + gv->trans->symlist[ t_sym ] );
         hmmcode = psym->hmm_code;
         hmm = GET_HMM( gv->base_hmms, hmmcode, hmm_dlt );


         /* Find the number of states in the HMM */
         n_state = NBR_STATES( hmm, gv->base_tran );

         /* Find the location of HMM state scores in the state cell array */
         p_state = STATE_CELL( gv->statecell_base, sym_cell->start_cell, n_state, frame_type, gv->max_state_cell );

         /* For now, just print the values of the state scores. Note that print
          out includes the exit state score which is a non-emitting state. This
          code should be modified to handle whatever processing is needed. */
         for( state = 0; state < n_state; state++ )
         {
            scr = p_state->scr;
            printf( "frame: %d, SYM: %d(%d),  HMM: %d(%d),  STATE: %d; SCR: %d\n",
               gv->frm_cnt, v_sym, t_sym, hmmcode+hmm_dlt, hmmcode, state, scr );
            p_state++;
         }
      }
   }
...
static void EM_alpha_0(short pd[], short alpha[], int T, HmmType *hmm, short c_t_beta, gmhmm_type *gv)
{
  ...
  FOR_EMS_STATES(i,hmm,gv->base_tran) {   /* initial A2: */ 
    ...
    tmp = (long)(HAS_TRANSITION(trans)? (trans + (long)pd[i * T]): BAD_SCR) - (long)c_t_beta;
