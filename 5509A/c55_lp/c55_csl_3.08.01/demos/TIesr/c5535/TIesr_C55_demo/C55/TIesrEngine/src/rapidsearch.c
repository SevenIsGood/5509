...
#else
     loadStatus = load_mean_vec(gv, FALSE); 
#endif
...
#else
  pconf->cm_anti_scr= (short *)MEM_alloc(SEG0, CM_MAX_FRM*sizeof(short), 0);
  if (pconf->cm_anti_scr == MEM_ILLEGAL) return eTIesrEngineCMMemorySize;
#endif
...
#else
  pconf->answers = (ushort *)MEM_alloc(SEG0, CM_MAX_WORD*sizeof(ushort), 0);
  if (pconf->answers == MEM_ILLEGAL) return eTIesrEngineCMMemorySize;
#endif
...
#else
  pconf->uStp = (ushort *)MEM_alloc(SEG0, CM_MAX_WORD*sizeof(ushort), 0);
  if (pconf->uStp == MEM_ILLEGAL) return eTIesrEngineCMMemorySize;
#endif
...
#else
  pconf->cm_scr = (short *)MEM_alloc(SEG0, CM_SCRS * sizeof(short), 0);
  if (pconf->cm_scr == MEM_ILLEGAL) return eTIesrEngineCMMemorySize;
#endif
...
#else
  pconf->lgBeta = (short *)MEM_alloc(SEG0, CM_NBR_BETA*sizeof(short), 0);
  if (pconf->lgBeta == MEM_ILLEGAL) return eTIesrEngineCMMemorySize;
#endif
...
#else
  pconf->acc_dif2_beta = (long *)MEM_alloc(SEG0, CM_NBR_BETA*sizeof(long), 2);
  if (pconf->acc_dif2_beta == MEM_ILLEGAL) return eTIesrEngineCMMemorySize;
#endif
...
#else
  pconf->sCH = (short *)MEM_alloc(SEG0, N_FILTER26*sizeof(short), 0);
  if (pconf->sCH == MEM_ILLEGAL) return eTIesrEngineCMMemorySize;
#endif
...
#else
  pconf->lAccum = (long *)MEM_alloc(SEG0, (2*N_FILTER26)*sizeof(long), 2);
  if (pconf->lAccum == MEM_ILLEGAL) return eTIesrEngineCMMemorySize;
#endif
...
#else
TIESRENGINECOREAPI_API void cm_load(gmhmm_type *gv)
{
#ifdef USE_CONFIDENCE
  ConfType * pconf = (ConfType*)gv->pConf; 
  short i; 
        ...
        short stp, long scaling)
{
  ...
  for (j=0, crt_vec = stt * feat_size; j<seg_len; j++, crt_vec += feat_size) {
    ...
    if (hmm_code == 0) {
      pConf->noise_power += i; 
      pConf->noise_power_sqr += (long)i * (long)i;/* Q12 */
    }
    else {
...
static short cm_logistic(short *x, gmhmm_type * gv)
{
  ...
  for (lsum=1;lsum<=CM_NBR_BETA-1;lsum++){
    ltmp2 = (long)(pConf->lgBeta[lsum]) * x[lsum-1]; /* Q12 */
  ...
  */
  ...
  ltmp = (long) utmp + ((long)1 << lsum);
  stmp = lsum - 9; 
  if (stmp > 0)
    ltmp = (long)Q15 / (ltmp >> stmp); /* to get Q6 output */
  else 
    ltmp = (long)Q15 / (ltmp << (-stmp));
...
static short cm_calc_proddiff(ConfType* pConf){
  ...
  ltmp = (long)(pConf->cm_scr[3]) * pConf->cm_scr[2]; /* Q12 */
...
short cm_score(gmhmm_type *gv)
{
  ...
  /* At present can only update status if JAC_update was successfully called. */
  if (pConf && pConf->bCalced == TRUE )
       return gv->cm_score; 
  else
     /* cm_update_status(gv); */
     return 0;
