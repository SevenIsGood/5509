        ...
        FOR_EACH_MIXING(k,pmix,pdf) { 
            ...
            if (Lr > gv->jac_component_likelihood ) {
                ...
                FOREACH(d,gv->n_filter) {
                    temp = (long)gamma_jk * func_d[d]; /*Q30*/
                                      ...
                                      Boolean bTriphone)
{
 ...
 Failure:
    ...
    unsigned short featDim;
    size_t numRead;
    TIesrEngineJACStatusType eStatus;
    ...
    eStatus = JAC_clear( gv, pNetDir, p_tree_file );
                                   ...
                                   const char *pNetDir, const char* p_tree_file)
{
    ...
    if (eStatus != eTIesrEngineJACSuccess) return eStatus; 
#endif
    return eTIesrEngineJACLoadFail;
#endif
    /* Load JAC state file, with hmm-to-phone map from data file. */
    eStatus = MP_JAC_load( bias_file, gv, pNetDir, p_tree_file, TRUE);
    return eStatus; 
...
TIesrEngineJACStatusType JAC_update(gmhmm_type *gvv)
{
    ...
    /* Can not do JAC_update if there was not enough memory to store mfcc vectors
     recognized. This test along with access to the stored vectors and counts 
     should be supplied outside of JAC_update to increase modularity. The 
     error condition is flagged as no alignment, since it is not possible to 
     do the alignment. */
...
#else
    if (load_mean_vec(gv, FALSE) != eTIesrEngineSuccess)
        return eTIesrEngineJACMemoryFail;
#endif
#endif
