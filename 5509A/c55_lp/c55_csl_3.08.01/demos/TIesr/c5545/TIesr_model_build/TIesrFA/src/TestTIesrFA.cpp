 ...
 *
 ...
 encoding = encoding type, (0->TIesrFALinear) or (3->TIesrFALinear_2)
...
int main( int argc, char** argv )
{
   ...
   if( argc > 9 )
   {
       encoding = (TIesrFA_Encoding_t)atoi( argv[9] );
       if( encoding != TIesrFALinear && encoding != TIesrFALinear_2 )
           return 10;
   }
   else
   {
       encoding = TIesrFALinear;
   }
   ...
   /* Number of bytes per mono or stereo sample for buffer sizing */
   switch (encoding )
   {
       case TIesrFALinear_2:
           sampleBytes = 4;
           break;
   ...
   sampleBuf = (unsigned char*) malloc( frameSamples*sampleBytes*sizeof(unsigned char) );

   error = TIesrFA_init( &aTIesrFAInstance, sampleRate, encoding, frameSamples,
   ...
   for( frameCount=0; frameCount<numFrames; frameCount++ )
   {
	 ...
	 return 4;
      ...
      fwrite( sampleBuf, sizeof( unsigned char), frameSamples*sampleBytes, fp );
