 ...
 *
 *  Modifications to operate on 16-Bit Plattforms. Add casts for
		   ...
		   short *reg_mfcc, short n_mfcc, const short *mu_scale_p2, short mfcc_buf[ /*MFCC_BUF_SZ*/ ][ MAX_DIM_MFCC16 ])
{
   ...
   if ( type == 0 ) {                  /* beg, no lag, simple diff */
	 ...
	 sum += j * (long)( mfcc_buf[ i_adv ][i] - mfcc_buf[ i_lag ][i] );
       }
...
#define accumulate_linear_mel_energy(tmp_l, tmp_s, mel_energy,  real_sig,  mel_filter) \
      tmp_l = (long)mel_filter * real_sig; /* Q14 x Q14 */ \
      tmp_s = (short) LONG_RIGHT_SHIFT_N( tmp_l, 14 ); /* Q14  */      \
    tmp_l = (long)tmp_s * tmp_s; /* Q 28 */                             \
...
mel_scale(const short *power_spectrum, long *mel_energy, const short n_filter, const mel_filter_type mel_filter[])
{
  ...
  for (i = 0; i < WINDOW_LEN / 2; i++) {
	...
	( (long)mel_filter[i].x2 * power_spectrum[i] );

    if ( cur_filter < n_filter )
      mel_energy[ cur_filter ] += (long) 
            ( (long)mel_filter[i].x1 * power_spectrum[i] );
  }
}
  ...
  for (i = 0; i < n_filter; i++) {
    ...
    for (j = 0; j < n_mfcc; j++) {
      ...
      /* Q 15 * Q 11 = Q 26, >> 17 = Q 9 ... see above*/
            tmp = (long)cosxfm[j][i] * mfcc[j];             /* transpose */
		      ...
		      const short n_mfcc, const short n_filter,  const short *cosxfm[  MAX_DIM_MFCC16 ])
{
  ...
  for (i = 0; i < n_mfcc; i++) {
	...
	sum += tmp>>( 7 + scale_p2[i] );
      ...
      {
	for (j = 0; j < n2; j++) {      	
                tmp = (long)cosxfm[i][j] * (log_mel_energy[j]- log_mel_energy[n_filter-1-j]) ;
		      ...
		      const short n_mfcc, const short n_filter,  const short *cosxfm[  MAX_DIM_MFCC16 ])
{
  ...
  for (i = 0; i < n_mfcc; i++) {
    ...
    for (j = 0; j < n_filter; j++) {
      ...
      /* Q 15 * Q 9 = Q 24, >> 13 = Q 11 */
            tmp = (long)cosxfm[i][j] * log_mel_energy[j];
      tmp += (long) 1 << ( 13 + scale_p2[i] - 1 );
      tmp >>= ( 13 + scale_p2[i] );
      sum += tmp;
    }
			  ...
			  const short *cosxfm[  MAX_DIM_MFCC16 ])
{
  ...
  /* log2() to log10() */
  for (i = 0; i < n_filter; i++) {
        tmp = (long) log_mel_energy[i] * LOG2TOLOG10;
...
fft(short *real, short *imag)
{
  ...
  while ( n_stage-- ) {
	...
	/* w_real and w_imag are Q15 DFT twiddle factor coefficients */
                r2 =(long) real[idx2] * (long)w_real[widx] - (long)imag[idx2] * (long)w_imag[widx]; /* Q 30 */
                i2 =(long) real[idx2] * (long)w_imag[widx] + (long)imag[idx2] * (long)w_real[widx];
...
static short hamming_window(short *sig)
{
  ...
  /* For scaling in case input sig is all zeros, set max
   to minimum value it could have if any sig were non-zero. */
  max = ((long)1<<13) * hw[0];

  /* find max */
  for (i = 0; i < WINDOW_LEN; i++) {
        tmp = (long) sig[i] * hw[i];             /* Q 15 * Q 15 = Q 30 */
    tmp = tmp > 0 ? tmp : (- tmp);
    if ( tmp > max ) max = tmp;
  }

  /* normalize to most precision */
  for( norm1 = 0; max < 0x40000000l; norm1++ )
    max <<= 1;
...
static short preemphasis(short *sig_in, short *sig_out, short *last_sig_0)
{
  ...
  {
    /* If sig_in is all zeros, set normalization factor as if max=1*/
    norm0 = 13;
  }

  tmp1 = ((long)last_sig << norm0) * PREEMPHASIS;          /* Q 15 * Q 15 = Q 30 */
  tmp2 = (long) (sig_in[0] << norm0) << 15;
  sig_out[0] =  LONG_RIGHT_SHIFT_N( tmp2 - tmp1, 15);  /* Q 15 */

  for (i = 1; i < WINDOW_LEN; i++) {
        tmp1 = ((long)sig_in[ i - 1 ] << norm0) * PREEMPHASIS;
...
static short fft_to_power_spectrum(const short *real, const short *imag, short *power_spectrum)
{
  ...
  long  tmp, max;
   ...
   /* For scaling in case input signal is all zeros resulting in all zero
   fft coefficients, set max to minimum value squared fft magnitude
   could have if any signal input were non-zero. */
  max = 20968 >> WINDOW_PWR;
  max = 2*max*max;

  /* find max */
  
  for (i = 0; i < WINDOW_LEN >> 1; i++) {
        tmp = (long) real[i] * real[i] + (long) imag[i] * imag[i];
    tmp <<= 1; /* Times 2 */
    if ( tmp > max ) max = tmp;
  }
  ...
  for (norm2 = 0; max < 0x40000000l; norm2++)
    max <<= 1;
...
#else
  { 
  	Int32 *sig32 = (Int32 *)real_sig;
  	Int16 *in, *out;
	Uint16 insig32;
	
    for( i = WINDOW_LEN-1; i >= 0; i-- )
    {
    	sig32[i] = ( (long)(real_sig[i]) << 16 );
    }
    
    hwafft_br( sig32, sig32_br, WINDOW_LEN );
 
    #if ( WINDOW_LEN == 256 )
 	/* 256 point complex fft with scaling */
    insig32 = hwafft_256pts( sig32_br, sig32, 0, 0 );
    #elif ( WINDOW_LEN == 512 ) 
    /* 512 point complex fft with scaling */
    insig32 = hwafft_512pts( sig32_br, sig32, 0, 0 ); 
    #endif

	/* Reorder the real/imag components into separate real/imag arrays */   
    if( insig32 )
    {
    	/* Must do in-place reorder. Probably best to use sig32_br
    	 * and then an extra copy because loops will not have
    ...
    {
    	out[i] = in[2*i];
    	out[WINDOW_LEN+i] = in[2*i+1];
    }  
    
    if( insig32 )
    {
    	for( i=0; i < 2*WINDOW_LEN; i++ )
    	{
    		in[i] = out[i];
    	}
    }
...
{
  #ifdef C55x_HWAFFT
  Uint32 cplx_sig[WINDOW_LEN];
  Int16 *real_sig = (Int16 *) &cplx_sig;
  Int16 *imag_sig = (Int16 *) &( cplx_sig[WINDOW_LEN/2] );
  #else
  short real_sig[ WINDOW_LEN ], imag_sig[ WINDOW_LEN ];
  #endif
  
  short norm, signal_abs_log2;
