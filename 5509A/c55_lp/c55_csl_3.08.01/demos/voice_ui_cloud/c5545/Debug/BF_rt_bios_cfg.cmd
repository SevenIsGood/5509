...
_audioTsk = audioTsk;
_TSK_idle = TSK_idle;
_uartdmaTsk = uartdmaTsk;
_uartrxTsk = uartrxTsk;
_oledTsk = oledTsk;
_TSK_blinkXGvoicerecog = TSK_blinkXGvoicerecog;
_TSK_Recognizer = TSK_Recognizer;
_LNK_dataPump = LNK_dataPump;
_RTA_dispatcher = RTA_dispatcher;
_LOG_system = LOG_system;
_SEM_i2s_dmaTransferDone = SEM_i2s_dmaTransferDone;
_SEM_UARTdataTransfer = SEM_UARTdataTransfer;
_SEM_uartrx_transferDone = SEM_uartrx_transferDone;
_SEM_OLEDTransfer = SEM_OLEDTransfer;
_SEM_BlinkXF = SEM_BlinkXF;
_SEM_DoneAudioCollection = SEM_DoneAudioCollection;
...
SECTIONS {
       ...
       /* no HWI stubs are necessary */
        ...
        .uartdmaTsk$stk: block(0x20000){
            *(.uartdmaTsk$stk)
        } > DARAM PAGE 0

        .uartrxTsk$stk: block(0x20000){
            *(.uartrxTsk$stk)
        } > DARAM PAGE 0

        .oledTsk$stk: block(0x20000){
            *(.oledTsk$stk)
        } > DARAM PAGE 0

        .TSK_blinkXGvoicerecog$stk: block(0x20000){
            *(.TSK_blinkXGvoicerecog$stk)
        } > DARAM PAGE 0

        .TSK_Recognizer$stk: block(0x20000){
            *(.TSK_Recognizer$stk)
        } > DARAM PAGE 0
