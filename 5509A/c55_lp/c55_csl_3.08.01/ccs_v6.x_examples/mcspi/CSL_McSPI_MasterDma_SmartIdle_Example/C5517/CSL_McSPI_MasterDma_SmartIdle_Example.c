...
/** @file CSL_McSPI_MasterDma_SmartIdle.c
 *
 *  @brief McSPI Example code to verify McSPI-host data transactions using DMA after being SMART IDLE'd
 *
 * \page    page10   McSPI EXAMPLE DOCUMENTATION
 *
 * \section McSPI4   McSPI EXAMPLE4 - MASTER SMART IDLE MODE
 *
 * \subsection McSPI4x    TEST DESCRIPTION:
 ...
 *
 * \subsection McSPI4y    TEST PROCEDURE:
 *
 ...
 *
 * \subsection McSPI4z    TEST RESULT:
...
CSL_FINST(CSL_SYSCTRL_REGS->EBSR, SYS_EBSR_SP1MODE, MODE1);
  ...
  else
  {
	...
	CSL_MCSPI_REGS->SYSCONFIGL |= (CSL_MCSPI_SYSCONFIGL_SIDLEMODE_MODE2<<3);
