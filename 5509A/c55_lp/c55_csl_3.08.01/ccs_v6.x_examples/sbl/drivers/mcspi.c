...
Uint16 MCSPI_INIT(unsigned int clock_divider)
{
	...
	CSL_FINS(CSL_MCSPI_REGS->CH0CONFL, MCSPI_CH0CONFL_WL,   CSL_MCSPI_CH0CONFL_WL_16BIT);
               ...
               unsigned short *cmdbuffer, unsigned short cmdcount, unsigned short readWrite )
{
	...
	if (readWrite==MCSPI_CMD_READ)
	{
		...
		// set to duplex mode to mode 0 duplex mode. we should get first clock here
		CSL_FINS(CSL_MCSPI_REGS->CH0CONFL, MCSPI_CH0CONFL_TRM,  CSL_MCSPI_CH0CONFL_TRM_MODE0);
		while (bufIndex<count)
		{
			// write 2 16-bit dummy word to provide the McSPI clock for receiving
			CSL_MCSPI_REGS->TX0L = (Uint16)(0x0000);
			CSL_MCSPI_REGS->TX0U = (Uint16)(0x0000);
			...
			// read one 16 bit data word
			buffer[bufIndex] = CSL_MCSPI_REGS->RX0L;
